...
def find_coordinate(resized_img, ensemble_mask_post_HF_EI):
	...
	segmentation_left_lung, segmentation_right_lung = None, None
	area_left_lung,area_right_lung = 0, 0
		...
		for i in range(int(IMAGE_SIZE[0])):  # y축 slicing
			...
			if i == int(IMAGE_SIZE[0])-1:
				...
				if x_max_coord < int(IMAGE_SIZE[1]*2/3) and x_min_coord < int(IMAGE_SIZE[1]*1/3):   # left lung
					...
					segmentation_left_lung = list(tmp_segmentation_list)
					area_left_lung = tmp_area
					...
					segmentation_right_lung = list(tmp_segmentation_list)
					area_right_lung = tmp_area
				else:
	...
	return (Region_img, mask_image_left_lung, mask_image_right_lung, color_mask_lung, 
			x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
			x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r,
			segmentation_left_lung, segmentation_right_lung,
			area_left_lung, area_right_lung)
...
def make_data_image(resized_img, mask_img_l, mask_img_r, iter, 
				x_min_l, y_max_l, x_max_l, y_min_l, x_min_r, y_max_r, x_max_r, y_min_r,
				segmentation_left_lung, segmentation_right_lung,
				area_left_lung, area_right_lung):
	...
	annotation = [
		{
			...
			"bbox" : [y_min_l, x_min_l, y_max_l ,x_max_l], # 좌측 하단, 우측 상단
...

		{
			...
			"bbox" : [y_min_r, x_min_r, y_max_r ,x_max_r], # 좌측 하단, 우측 상단
...
def Separation_l_r_lung(ensemble_mask_post_HF_EI, img_, path_, iter):
	...
	(Region_img, mask_img_l, mask_img_r, color_mask_lung, 
	x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
	x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r,
	segmentation_left_lung, segmentation_right_lung,
	area_left_lung, area_right_lung) = find_coordinate(resized_img, ensemble_mask_post_HF_EI)
	...
	data_image = make_data_image(resized_img, mask_img_l, mask_img_r, iter,
				x_min_l, y_max_l, x_max_l, y_min_l, x_min_r, y_max_r, x_max_r, y_min_r,
				segmentation_left_lung, segmentation_right_lung,
				area_left_lung, area_right_lung)
...
def main(_):
	...
	for iter, path_ in enumerate(sorted(glob.glob (path_base_input + '\*.*'))):		 
		...
		data_image = Separation_l_r_lung(ensemble_mask_post_HF_EI, img, path_, iter)
