...
path_base_input = os.path.join(os.getcwd() , 'input_dataset')  

path_base_result = os.path.join(os.getcwd() , 'result_img')
...
def draw_GT_box(resized_img, 
				x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
				x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r):

	pt1_l, pt2_l = (x_min_l, y_max_l), (x_max_l, y_min_l)
	cv2.rectangle(resized_img, pt1_l, pt2_l, color = (255, 0, 0), thickness = None, lineType = None, shift =None)
	cv2.circle(resized_img, (x_center_l, y_center_l), radius = 3, color = (255, 0, 0), thickness = -1)

	pt1_r, pt2_r = (x_min_r, y_max_r), (x_max_r, y_min_r)
	cv2.rectangle(resized_img, pt1_r, pt2_r, color = (0, 0, 255), thickness = None, lineType = None, shift =None)
	cv2.circle(resized_img, (x_center_r, y_center_r), radius = 3, color = (0, 0, 255), thickness = -1)

	return resized_img

def make_dataframe(resized_img, mask_img, 
				x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
				x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r):

	# save file as .csv
	data_left_lung = [[1, 'left_lung', [x_center_l, y_center_l], [x_min_l, y_min_l, x_max_l, y_max_l]]]
	columns_left_lung = ['class_ids', 'class_name', 'center_coordinate_x_y', 'Bbox_coordinate']
	left_lung = pd.DataFrame(data_left_lung, columns = columns_left_lung)
	
	data_right_lung = [[1, 'right_lung', [x_center_r, y_center_r], [x_min_r, y_min_r, x_max_r, y_max_r]]]
	columns_right_lung = ['class_ids', 'class_name', 'center_coordinate_x_y', 'Bbox_coordinate']
	right_lung = pd.DataFrame(data_right_lung, columns = columns_right_lung)
	
	data_labels = [[left_lung, right_lung]]
	columns_data_labels = ['left_lung', 'right_lung']
	labels = pd.DataFrame(data_labels, columns= columns_data_labels)

	data_dataset = [[resized_img, mask_img, labels]]
	columns_dataset = ['original_image', 'mask_image', 'labels']
	dataset = pd.DataFrame(data_dataset, columns=columns_dataset)

	return dataset


def save_fig_images(mask_img, resized_img, res_image_lung, original_img, path_, iter):
...
def Separation_l_r_lung(ensemble_mask_post_HF_EI, img_, path_, iter):
	...
	(mask_img, color_mask_lung, 
	x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
	x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r) = find_coordinate(resized_img, ensemble_mask_post_HF_EI)
	...
	save_fig_images(mask_img, GT_image_img, res_image_lung, original_img, path_, iter)

	# image로부터 뽑은 infomation으로 dataset을 만든다.
	dataset = make_dataframe(resized_img, mask_img, 
				x_min_l, y_max_l, x_max_l, y_min_l, x_center_l, y_center_l,
				x_min_r, y_max_r, x_max_r, y_min_r, x_center_r, y_center_r)
...
def main(_):
	...
	for iter, path_ in enumerate(sorted(glob.glob (path_base_input + '\*.*'))):		 
		...
		dataset = Separation_l_r_lung(ensemble_mask_post_HF_EI, img, path_, iter)

		dataset.append(dataset, ignore_index = True)
	dataset.to_csv(path_save_result_dataset + '\dataset_lung_segmentation.csv')
